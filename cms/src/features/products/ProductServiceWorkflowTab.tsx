...
/**
 * Product/Service Workflow tab (spec §12). Status + role-gated transitions
 * against the real governed endpoints (EDITOR+ submits, REVIEWER+
 * approves/publishes, SUPER_ADMIN archives) and the version history the
 * backend records on every transition. The backend remains the permission
 * boundary.
 */
export function ProductServiceWorkflowTab({ productId }: { productId: string }) {
  const { user } = useAuth();
  ...
  const detail = useQuery({
    queryKey: ['admin-product-services', productId],
    queryFn: () => productServiceApi.get(productId),
  });

  const invalidate = () => {
    void queryClient.invalidateQueries({ queryKey: ['admin-product-services'] });
    void queryClient.invalidateQueries({ queryKey: ['admin-product-services', productId] });
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardContent className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <PreviewLink route="product-services" id={row.id} />
            {canEdit(role) && row.status === 'DRAFT' && (
              <Button
                variant="outline"
                loading={busy}
                onClick={() => void run(() => productServiceApi.submit(row.id), 'Submitted for review')}
              >
                Submit for review
              </Button>
            )}
            {canReview(role) && row.status === 'IN_REVIEW' && (
              <Button
                variant="outline"
                loading={busy}
                onClick={() => void run(() => productServiceApi.approve(row.id), 'Approved')}
              >
                Approve
              </Button>
            )}
            {canReview(role) && row.status === 'APPROVED' && (
              <Button
                variant="outline"
                loading={busy}
                onClick={() => void run(() => productServiceApi.publish(row.id), 'Published')}
              >
                Publish
              </Button>
            )}
      ...
      <ConfirmDialog
        ...
        description={`This removes "${row.name}" from the published site and closes its workflow. This cannot be undone from the CMS.`}
        confirmLabel="Archive"
        tone="danger"
        loading={busy}
        onConfirm={() => {
          setArchiveOpen(false);
          void run(() => productServiceApi.archive(row.id), 'Archived').then(() => navigate('/app/products'));
        }}
