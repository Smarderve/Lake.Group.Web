...
/**
 * Company Workflow tab (spec §10). Status + role-gated transitions against
 ...
 */
export function CompanyWorkflowTab({ companyId }: { companyId: string }) {
  const { user } = useAuth();
    ...
    <div className="space-y-4">
      <Card>
        <CardContent className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <PreviewLink route="companies" id={row.id} />
            {canEdit(role) && row.status === 'DRAFT' && (
              <Button
                variant="outline"
                loading={busy}
                onClick={() => void run(() => companyApi.submit(row.id), 'Submitted for review')}
              >
                Submit for review
              </Button>
            )}
            {canReview(role) && row.status === 'IN_REVIEW' && (
              <Button
                variant="outline"
                loading={busy}
                onClick={() => void run(() => companyApi.approve(row.id), 'Approved')}
              >
                Approve
              </Button>
            )}
            {canReview(role) && row.status === 'APPROVED' && (
              <Button
                variant="outline"
                loading={busy}
                onClick={() => void run(() => companyApi.publish(row.id), 'Published')}
              >
                Publish
              </Button>
            )}
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          ...
          void run(() => companyApi.archive(row.id), 'Archived').then(() => navigate('/app/companies'));
        }}
