...
import { componentRegistry, compositionDiff, compositionFromLegacy, duplicateNode, effectiveLayout, insertNode, makeNode, moveNode, removeNode, resetResponsive, setResponsiveLayout, updateNode } from '../src/features/control-center/composition';

const legacy = () => ({
  hero: { heading: 'Lake Aviation', description: 'Reliable fuel', image: '/hero.webp', alt: 'Aircraft' },
  introduction: { heading: 'About', body: 'Operations across East Africa.' },
  cta: { label: 'Contact', href: '/contact.html' },
  sections: [{ key: 'services', heading: 'Services', body: 'Into-plane fueling' }],
});
...
describe('CMS V2 composition engine', () => {
  it('registers production component families and creates a structured migration', () => {
    ...
    expect(composition.root.children.map((node) => node.type)).toEqual(['company-hero', 'image-text', 'service-card', 'cta']);
  ...
  it('supports insert, duplicate, reorder, hide and delete with protected nodes', () => {
    ...
    composition = updateNode(composition, card.id, (node) => { node.visible = false; });
    expect(composition.root.children.find((node) => node.id === card.id)?.visible).toBe(false);
    composition = removeNode(composition, card.id);
    expect(composition.root.children.some((node) => node.id === card.id)).toBe(false);
  ...
  it('rejects invalid nesting and reports structured move/layout/visibility differences', () => {
    ...
    expect(() => insertNode(before, makeNode('section'), 'hero')).toThrow(/cannot be inserted/);
    let after = moveNode(before, 'section-services', -1);
    after = updateNode(after, 'section-services', (node) => { node.layout.span = 6; node.visible = false; });
    const changes = compositionDiff(before, after);
    expect(changes.some((change) => change.kind === 'moved')).toBe(true);
    expect(changes.some((change) => change.label.includes('layout'))).toBe(true);
    expect(changes.some((change) => change.label.includes('visible'))).toBe(true);
  });
});
