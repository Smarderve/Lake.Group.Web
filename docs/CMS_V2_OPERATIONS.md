...
The current entry point is `/control`. `/app` remains the default and retains existing workflows. The preview source defaults to `https://lake-group.vercel.app`; set `CMS_V2_PUBLIC_SITE_ORIGIN` on the backend to another HTTPS public origin when necessary. The CMS CSP must permit that origin for base, styles, fonts and images. A release created through `/control` is an immutable CMS artifact; verify its downstream deployment separately because public delivery is not connected yet.
