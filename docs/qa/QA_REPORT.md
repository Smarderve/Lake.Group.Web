...
- **Leadership names and tenures** — `leadership.html` names executives (e.g. Juma Nuru) beyond the founder. Per the research dataset (`scripts/_verified_lake_facts.md`), **only Ally Edha Awadh (Founder & Chairman) is verifiable from official Lake Group sources**; other executive names/titles/tenures could not be confirmed and must be validated by the client before launch.
