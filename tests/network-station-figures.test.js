...
test('canonical metrics use 500+ fuel stations and no longer show Across Africa stat', () => {
  const seeds = read('backend/scripts/seed-metrics.js');

  assert.match(seeds, /key: 'stations',[\s\S]*?label: 'Fuel Stations',[\s\S]*?value: '500\+'/);
  // The network_locations / Across Africa stat card has been removed
  assert.doesNotMatch(seeds, /key: 'network_locations'/);
});

test('home uses 500+ fuel stations and no longer shows Across Africa stat card', () => {
  const home = read('index.html');

  assert.match(home, /data-metric-key="stations">500\+<\/span>/);
...
test('Home presents 500+ fuel stations without overflow', async () => {
  ...
  try {
    ...
    for (const viewport of [{ width: 1440, height: 900 }, { width: 820, height: 1180 }, { width: 390, height: 844 }]) {
      ...
      await page.waitForFunction(() => document.querySelector('[data-metric-key="stations"]')?.textContent.trim() === '500+', null, { timeout: 5000 });
      const metrics = await page.locator('.hero-kf').allTextContents();
      assert.ok(metrics.some((value) => /500\+\s*Fuel Stations/i.test(value)), `missing 500+ fuel stations at ${viewport.width}px`);
