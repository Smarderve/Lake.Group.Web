...
function schema(file, pageTitle, pageDescription) {
  if (!SITE.isConfigured || !indexable.has(file)) return null;
...
function managedHead(file, pageTitle, pageDescription) {
  const canPublish = SITE.isConfigured && indexable.has(file);
  const lines = [
    '<!-- SEO:START (generated by scripts/build-seo-foundation.mjs) -->',
    `<meta name="description" content="${pageDescription.replace(/"/g, '&quot;')}">`,
    `<meta name="robots" content="${canPublish ? 'index,follow,max-image-preview:large' : 'noindex,nofollow'}">`,
  ...
  if (SITE.isConfigured) {
    ...
    lines.push(`<meta name="twitter:image" content="${SITE.origin + SITE.socialImage}">`);
