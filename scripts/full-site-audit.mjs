...
const keyPages = ['index.html', 'about.html', 'leadership.html', 'history.html', 'gallery.html', 'contact.html', 'careers.html', 'csr.html'', 'lake-oil.html', 'lake-gas.html', 'lake-lubes.html', 'lake-agro.html', 'aficd.html', 'lake-trans.html', 'lake-steel.html', 'lake-premix-cement.html', 'station-locator.html', 'under-construction.html'].filter(f => rootPages.includes(f));

function serve() {
  const server = http.createServer((req, res) => {
    let requested = decodeURIComponent(req.url.split('?')[0]).replace(/^\/+/, '') || 'index.html';
    // Mirror the production redirects for legacy direct-access URLs so the
    // local crawl measures the canonical destination, not an obsolete shell.
    if (requested === 'la-home.html' || requested === 'la-projects.html') requested = 'lake-agro.html';
...
async function inspect(browser, origin, pageName, viewport) {
  ...
  if (!data) throw new Error(`Unable to inspect ${pageName} after navigation settled`);
  await context.close();
...
await runJobs(localJobs, local);
if (!localOnly) await runJobs(liveJobs, live);
const engines = {};
...
const report = { generatedAt: new Date().toISOString(), liveOrigin, localOrigin, localOnly, rootPages, keyPages, engines, inventory: inventory(), local, live };
